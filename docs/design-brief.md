# Design Brief — Accurate Giant Company Ltd. website
...
- **Palette:** `--brand-primary: #013299` (deep blue) and `--brand-secondary: #00B9EF` (cyan), blended with a confirmed grey ramp (`--brand-ink` through `--brand-border-strong`). State colours remain DRAFT. The cyan **fails AA on white for body text** — reserve for accents and inverse use.
