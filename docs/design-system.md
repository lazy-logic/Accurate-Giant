# Design System Foundations — Accurate Giant Company Ltd.
...
| Body / UI (everything else) | **Inter** | `next/font/google` | 400, 500, 600, 700 |
| Numerals (results page) | **Inter** with `font-feature-settings: "tnum" 1, "lnum" 1` | inline class | 600 |

> Montserrat delivers the bold, confident, modern presence the brand now wants — geometric sans paired with Inter for body. Defaults are intentionally heavy (h1 800, h2/h3 700) because Montserrat at 400 reads thin at display sizes. See [ADR 0009](decisions/0009-montserrat-inter-type-pair.md) for the move from Fraunces.

Both are exposed as CSS variables: `--font-display` and `--font-sans`. Components reference variables, never the literal family.
...
Display headlines (`text-4xl` and up) use Montserrat. Default weights via the heading element type — h1 weight 800, h2/h3 weight 700. Override via `font-bold`, `font-extrabold`, `font-black` Tailwind utilities when a non-heading needs the same treatment (e.g. a `<p>` with `font-display font-extrabold text-7xl` for the GHS giving figure).

### 1.3 Line-height pairings

| Type role | Family | Line-height |
|-----------|--------|-------------|
| Mega display (`text-7xl`/`text-8xl`) | Montserrat | 1 |
| Hero / page titles (h1) | Montserrat | 1 |
| Section headings (h2) | Montserrat | 1.05 |
| Sub-heads (h3) | Montserrat | 1.15 |
| Body (`text-base`/`text-lg`) | Inter | 1.55 |
| UI labels and buttons | Inter | 1.2 |
